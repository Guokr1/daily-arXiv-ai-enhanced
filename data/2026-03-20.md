...
TL;DR: 在薄膜铌酸锂平台上集成声光波束转向与光学环形谐振器，实现了20%的共振增强转向效率，并展示了FMCW LiDAR应用。


<details>
  <summary>Details</summary>
Motivation: 传统机械波束转向系统扫描速度有限且体积庞大，现有固态解决方案依赖像素化合成孔径，制造和控制复杂。需要开发紧凑、高效、可扩展的波束转向平台。

Method: 在薄膜铌酸锂平台上集成声光波束转向与光学环形谐振器，利用铌酸锂的压电和电光特性，通过集成电光控制将谐振器共振锁定到啁啾激光频率。

Result: 实现了高达20%的共振增强波束转向效率，覆盖18度视场，并演示了频率调制连续波LiDAR操作。
...
TL;DR: 提出一种无需基底的SUEX干膜环氧树脂光刻方法，直接制造独立微颗粒，产量接近100%，支持复杂几何形状，结合Nazca Python库实现大规模参数化设计。


<details>
  <summary>Details</summary>
Motivation: 传统SU-8颗粒制造需要在固体基底上图案化，然后通过牺牲层释放，这种方法效率低且需要额外步骤。本研究旨在开发一种无需基底的直接制造方法，提高产量并简化工艺。

Method: 使用SUEX干膜环氧树脂光刻技术，完全消除基底使用，直接制造独立微颗粒。结合Nazca Python库实现程序化设计，能够生成数万个参数化定义的颗粒设计。

Result: 实现了接近100%的颗粒产量，支持高宽比和高度复杂的平面几何形状。该方法为大规模颗粒库的制造提供了可扩展的途径。

Conclusion: 这种无基底制造与自动化设计的结合，为材料科学、微流体和软物质应用中的定制微颗粒提供了可扩展的制造路线。
...
TL;DR: 研究液态金属-铝混合机器在空间受限条件下的运动动力学，揭示非对称供能原理如何影响方向调控、速度控制和寿命，为自主机器人应用提供理论基础


<details>
  <summary>Details</summary>
Motivation: 液态金属机器在自主运动系统中具有潜力，但空间受限如何影响其运动动力学、速度方向控制和寿命等基本问题尚未充分探索，理解这些受限动力学对实际应用至关重要

Method: 将液态金属机器限制在一维半开放通道中，使用超高速度成像技术，系统研究不同末端障碍物对其整个生命周期的影响和转向动力学，重点关注燃料区域形态演化、整体运动和局部流动特性

Result: 揭示了燃料区域演化和末端障碍物相互作用如何影响对称性破缺机制和往复动力学，发现了材料特性、电荷转移过程和流体动力学在末端转向过程中的复杂相互作用

Conclusion: 建立了液态金属机器驱动动力学的理论基础，并证明其具有高效的热质传递能力，为受控运输系统和自主机器人应用开辟了新途径
...
### [4] [Stability of Charge Collection Efficiency and Time Resolution in 4H-SiC PIN Diodes Under X-ray Irradiation](https://arxiv.org/abs/2603.18547)
*Jiaqi Zhou,Sen Zhao,Xiyuan Zhang,Suyu Xiao,Chenxi Fu,Congcong Wang,Yanpeng Li,Weimin Song,Xin Shi*

Main category: physics.ins-det

TL;DR: 4H-SiC PIN探测器在2 MGy X射线辐照下表现出优异的抗辐射性能，泄漏电流极低，电荷收集效率下降小于5%，时间分辨率保持良好。


<details>
  <summary>Details</summary>
Motivation: 评估4H-SiC PIN探测器在极端X射线辐照下的抗辐射性能，为高能物理、空间任务和核反应堆监测等辐射硬化应用提供可靠器件。

Method: 采用全外延垂直PIN结构，带台面终端和场板。在160 keV能量下进行高达2 MGy(Si)的X射线辐照，辐照前后进行I-V/C-V测量、β粒子(⁹⁰Sr)电荷收集效率(CCE)和时间分辨率测试。

Result: 辐照后反向泄漏电流保持在~10⁻¹¹ A/cm²的超低水平，C-V特性基本一致，完全耗尽电压约130V。β粒子电荷收集效率下降小于5%，时间分辨率从21ps变为31ps，抖动适度增加。

Conclusion: 4H-SiC PIN探测器在极端X射线辐照下表现出稳定的性能，展示了其在辐射硬化应用中的巨大潜力。
...
### [5] [Spatial resolution improvement of PICOSEC Micromegas precise timing detectors](https://arxiv.org/abs/2603.18733)
...
TL;DR: PICOSEC Micromegas探测器通过高读取粒度实现约0.5mm空间分辨率，同时保持优于20ps的时间分辨率，可作为精确定时和中分辨率追踪探测器


<details>
  <summary>Details</summary>
Motivation: 研究高读取粒度的PICOSEC Micromegas原型，以改进空间分辨率，同时保持优异的时间分辨率，使探测器能够同时用于精确定时和中分辨率追踪

Method: 使用切伦科夫辐射体、半透明光阴极和Micromegas放大级组合，研究不同读取垫尺寸（3.5mm和2.2mm）的高粒度原型，评估空间和时间分辨率

Result: 3.5mm读取垫实现约0.5mm空间分辨率，进一步减小到2.2mm未显著改善；中等粒度原型的时间分辨率略有下降但仍优于20ps

Conclusion: PICOSEC Micromegas探测器可实现约0.5mm空间分辨率和优于20ps时间分辨率，适合同时作为精确定时和中分辨率追踪探测器
...
### [6] [A Sub-electron-noise Skipper-CCD Readout ASIC with Improved Channel-to-channel Isolation and an Integrated Cryogenic Voltage Reference](https://arxiv.org/abs/2603.18993)
...
TL;DR: MIDNA ASIC系列芯片采用65nm低功耗CMOS工艺制造，专为skipper-CCD读出设计，实现了基于双斜率积分器的相关双采样信号处理链，可在室温至84K低温下工作。最新版本改进了设计并增加了片上电压基准，性能显著提升。


<details>
  <summary>Details</summary>
Motivation: 开发能够在低温环境下（低至84K）工作的skipper-CCD读出芯片，解决现有设计中的漏洞，提高单电子分辨率并降低读出噪声，满足高精度电荷测量需求。

Method: 采用65nm低功耗CMOS工艺制造ASIC芯片，实现基于双斜率积分器的相关双采样信号处理链。通过设计更新和增加片上电压参考来改进性能，使用模拟堆积技术在模拟域平均测量电荷。

Result: 在140K温度下与skipper-CCD配合测试，通过1200个样本的平均，读出噪声低至0.11 erms等效电荷，达到单电子分辨率。通道间串扰优于-62 dB，性能显著改善。

Conclusion: MIDNA ASIC经过设计改进后，在低温环境下实现了优异的性能，能够达到单电子分辨率，低噪声和低串扰特性使其适用于高精度电荷测量应用。
...
### [7] [A Minimal-Component 100 MHz Full-Duplex Digital Link Over a Single Coaxial Cable for Laboratory Instrumentation](https://arxiv.org/abs/2603.18848)
*Michael Wiebusch*

Main category: physics.ins-det

TL;DR: 提出一种使用单根同轴电缆实现双向数字信号传输的极简电路，仅需无源电阻混合网络、单个CMOS逻辑门驱动器和商用LVDS接收器，无需主动回声消除或变压器耦合。


<details>
  <summary>Details</summary>
Motivation: 在实验室和探测器环境中，电缆布线或馈通密度受限，需要一种简单且兼容现有同轴电缆基础设施的双向数字互连方案。

Method: 采用无源电阻混合网络提供匹配线端接和方向分离，单个CMOS逻辑门作为驱动器，商用LVDS接收器作为差分比较器。通过分析混合网络确定最大化接收信号幅度的系统参数。

Result: 对于6米以内的典型实验室同轴电缆，测量到的峰峰值边缘定时误差低于1ns。250 MBaud随机数据的双向传输实验显示清晰的眼图，证实了可靠的全双工操作。

Conclusion: 该方案因其简单性和与现有同轴基础设施的兼容性，在电缆布线或馈通密度受限的实验室和探测器环境中具有应用潜力。
...
### [8] [Setting SAIL: Leveraging Scientist-AI-Loops for Rigorous Visualization Tools](https://arxiv.org/abs/2603.18145)
...
TL;DR: 科学家提出SAIL框架，通过分离领域逻辑与代码语法，让研究人员在保持科学严谨性的同时利用AI加速代码生成，并开发了两个天体物理学工具进行验证。


<details>
  <summary>Details</summary>
Motivation: 科学家面临理论知识与构建交互工具所需专业技能之间的鸿沟，而现有LLM生成的代码往往注重功能语法而忽视科学准确性，可能导致视觉上吸引人但科学上无效的结果。

Method: 提出Scientist-AI-Loop (SAIL)框架，将领域逻辑与代码语法分离，研究人员保持对科学概念和约束的严格监督，同时将代码实现委托给AI。通过两个开源浏览器天体物理学工具（引力透镜可视化和大尺度结构形成沙盒）进行验证。

Result: SAIL方法将开发时间缩短至数天，同时保持科学完整性。成功解决了AI生成代码忽视现象学边界或科学有效性的失败模式，为外展、教学、专业演示和早期研究原型提供了有效的代码生成工作流程。

Conclusion: 虽然研究级代码需要严格协议，但SAIL框架为AI辅助科学软件的进一步发展奠定了基础，展示了在保持科学严谨性的同时利用AI加速代码生成的可行性。
...
### [9] [ALABI: Active Learning for Accelerated Bayesian Inference](https://arxiv.org/abs/2603.18259)
*Jessica Birky,Rory K. Barnes*

Main category: astro-ph.IM

TL;DR: alabi是一个用于加速贝叶斯推断的Python包，使用高斯过程代理模型和主动学习来减少计算昂贵模型评估次数，可加速MCMC计算10-1000倍。


<details>
  <summary>Details</summary>
Motivation: 贝叶斯推断中，当正向模型计算成本高昂时（如评估时间≥1秒），传统的MCMC方法需要大量模型评估，计算效率低下。需要一种方法来减少昂贵模型评估次数，同时保持推断准确性。

Method: 使用高斯过程（GP）作为代理模型来预测后验概率，结合主动学习策略迭代改进GP在高似然区域的不确定性预测。提供统一接口支持多种采样器（emcee, dynesty, multinest, ultranest）。

Result: 在测试案例中，alabi能够将计算昂贵模型评估次数减少数千倍，对评估时间≥1秒的似然函数，MCMC计算速度提升10-1000倍，最高支持64维问题。

Conclusion: alabi为计算昂贵的贝叶斯推断问题提供了有效的加速解决方案，通过GP代理模型和主动学习显著减少计算成本，同时保持推断准确性。
...
### [10] [cosmo-numba: B-modes and COSEBIs computations accelerated by Numba](https://arxiv.org/abs/2603.18419)
...
Method: 开发了一个基于Python的软件包，实现了E/B模式分解算法，优化了数值精度和计算速度

Result: 该实现具有高精度和高速度，避免了数值精度或计算时间成为显著限制因素

Conclusion: 该软件包能够满足弱引力透镜分析中对E/B模式分解的需求，适用于系统误差检查和科学分析
...
TL;DR: 使用日本3.8米Seimei望远镜上的IMONY高时间分辨率光子计数成像仪，成功探测到蟹状星云脉冲星的光学单脉冲，并发现光学峰值时间在三天内漂移30±7.9微秒，对应发射区域尺寸约9.1公里。


<details>
  <summary>Details</summary>
Motivation: 研究蟹状星云脉冲星的光学脉冲发射特性，特别是单脉冲的时间变化和发射区域的稳定性，以理解脉冲星磁层物理和发射机制。

Method: 使用安装在3.8米Seimei望远镜上的IMONY光子计数成像仪（100纳秒时间分辨率），进行为期一周的夜间光学观测，其中两晚与64米Usuda射电望远镜进行同步光学-射电观测。

Result: 成功探测到每个旋转周期发射的光学单脉冲；发现光学峰值时间在三天内漂移30±7.9微秒（3.9σ显著性）；对应发射区域尺寸为9.1公里，相当于蟹状星云脉冲星光柱半径的0.006倍。

Conclusion: 排除了脉冲星glitch的可能性，表明蟹状星云脉冲星的光学脉冲发射区域可能因磁层焦散的空间漂移和变化而波动。
...
TL;DR: 使用FAST望远镜对星际天体3I/ATLAS进行窄带无线电技术特征搜索，未发现可信信号，对潜在发射器功率设置上限约束


<details>
  <summary>Details</summary>
Motivation: 3I/ATLAS是第三个确认的穿越太阳系的星际天体，为寻找地外文明技术特征提供了独特机会。星际天体可能携带或关联地外技术设备，对其进行无线电观测可以探测可能的技术信号

Method: 使用FAST望远镜L波段多波束接收器，在2025年10月至2026年1月的4个关键时间点对3I/ATLAS进行观测。采用bliss流水线在1.05-1.45 GHz频率范围内搜索信噪比超过10的频率漂移信号，通过波束、频率和漂移率匹配将信号分组为事件，然后通过聚类分析和漂移率截断进行过滤，并使用信噪比、结构张量和主成分分析对事件进行特征分析

Result: 未检测到可信的窄带无线电技术特征信号。零结果对发射器存在设置了功率上限约束：超过2.862×10⁻³ W的发射器被排除。进一步通过贝叶斯推断框架，使用物理动机先验来约束假设发射器的存在概率和特征功率

Conclusion: 虽然未发现技术特征信号，但研究建立了对星际天体进行技术特征搜索的方法论，并为未来类似观测设置了灵敏度基准。贝叶斯框架为评估假设发射器参数提供了统计工具
...
TL;DR: 提出一种D4等变深度神经网络用于星系形状测量，结合解析校准框架，在LSST模拟中比传统方法降低10%形状噪声，同时满足LSST的校准要求。


<details>
  <summary>Details</summary>
Motivation: 传统弱引力透镜剪切估计器难以完全捕捉真实星系形态、PSF效应、混合和噪声，而盲目训练的机器学习模型会引入显著校准偏差，需要一种既能利用机器学习优势又能保证校准精度的方法。

Method: 构建完全D4等变的深度神经网络（具有90度旋转和镜像变换对称性），采用解析校准框架（AnaCal）通过反向传播梯度校准模型。

Result: 在LSST模拟中，高噪声区域比传统傅里叶功率函数形状基方法降低约10%形状噪声（相当于增加20%有效星系数密度），乘性偏差满足|m|<10^-3（LSST要求），多数达到10^-4水平。

Conclusion: 该方法为未来基于机器学习的剪切估计扩展到混合源和多波段观测建立了严格的物理基础，所有代码和数据将在接受后公开。
...
TL;DR: 提出一种高效率、高分辨率的片上滤波器组光谱仪，用于线强度映射和宽带波状暗物质搜索，通过消除终端电阻和使用铌-硅共面波导谐振结构，实现超过74%的通道效率和R=1211的分辨率。


<details>
  <summary>Details</summary>
Motivation: 现有超导滤波器组架构存在50%固有效率限制和对薄膜介电损耗高度敏感的问题，限制了毫米波光谱实验的灵敏度，需要开发更高效、更稳健的设计方案。

Method: 采用消除终端电阻的设计，使用铌-硅共面波导谐振结构构建滤波器组，通过Sonnet电磁仿真验证10通道90GHz设备的性能。

Result: 仿真显示单通道效率超过74%，分辨率R=1211±105，对典型制造不确定性具有鲁棒性（除介电厚度外），为下一代毫米波光谱实验提供可扩展技术方案。

Conclusion: 该设计成功解决了现有滤波器组的效率瓶颈和损耗敏感性问题，在线强度映射和暗物质搜索应用中展现出优越性能，具有实际应用潜力。
...
### [15] [Trapped Proton Environment in Medium-Earth Orbit (2000-2010)](https://arxiv.org/abs/2603.18352)
*Yue Chen,Reinhard H. W. Friedel,Richard M. Kippen*

Main category: physics.space-ph

TL;DR: 开发了一种基于Polar卫星数据的新质子辐射带模型，结合GPS原位测量，生成了2000-2010年期间GPS轨道上的质子通量数据，覆盖50keV-6MeV能量范围，并包含通量统计信息。


<details>
  <summary>Details</summary>
Motivation: 现有AP8质子辐射带模型仅提供平均值，且基于较旧数据，无法反映质子带的动态变化和统计特性。需要开发基于新数据集的模型来更准确地描述中地球轨道上的质子环境。

Method: 1. 基于Polar卫星CEPPAD质子测量数据（1996-2007）开发新的经验质子辐射带模型；2. 将模型输出与GPS ns41原位测量数据进行比例缩放；3. 生成覆盖50keV-6MeV能量范围的每日质子通量数据。

Result: 新模型与AP8模型结果差异显著，表明AP8常用的误差因子2过于简化，低估了质子带的变化。新模型误差因子<3，原位测量误差因子约5。生成了GPS轨道上的每日质子通量数据集。

Conclusion: 新开发的质子辐射带模型结合GPS原位测量，能够更好地描述中地球轨道上的质子环境动态变化，提供更全面的统计信息（最坏情况、发生百分位数），对空间天气和航天器防护有重要意义。
...
### [16] [More on near-horizon charges black holes with gravitational hair in three dimensions](https://arxiv.org/abs/2603.18072)
*Seyed Naseh Sajadi,Supakchai Ponglertsakul,Julio Oliva*

Main category: gr-qc

TL;DR: 本文系统研究了高阶曲率引力理论（最高至四阶黎曼张量项）的协变相空间方法，应用于弦理论α'修正和旋转BTZ时空的近地平线对称性与守恒荷计算，特别关注具有引力毛发的黑洞情况。


<details>
  <summary>Details</summary>
Motivation: 探索高阶曲率引力理论中的近地平线守恒荷，寻找具有普适行为的理论部分，特别关注弦理论α'修正（如Type II理论中的α'³R⁴项）和具有引力毛发的黑洞情况。

Method: 1. 建立适用于任意维度、包含最高四阶黎曼张量项的引力理论的协变相空间方法公式体系；2. 在旋转BTZ时空中测试公式并验证黑洞热力学第一定律；3. 研究New Massive Gravity等具有引力毛发的黑洞理论的近地平线渐近对称性；4. 计算静态和旋转毛发黑洞的守恒荷，将之前仅到二次项的工作扩展到四次项。

Result: 1. 建立了完整的协变相空间方法公式体系；2. 在BTZ时空中成功恢复了黑洞热力学第一定律；3. 研究了具有四个最大对称真空的理论（如New Massive Gravity）的近地平线对称性；4. 计算了静态和旋转毛发黑洞的守恒荷，将分析扩展到四次曲率项。

Conclusion: 本文系统扩展了高阶曲率引力理论的协变相空间方法，为研究弦理论α'修正和具有引力毛发的黑洞提供了完整框架，特别在近地平线对称性和守恒荷计算方面取得了进展，将分析从二次项扩展到四次项，为理解这些理论中的普适行为奠定了基础。
...
### [17] [On the concept of simultaneity in relativity](https://arxiv.org/abs/2603.18100)
*Justo Pastor Lambare*

Main category: gr-qc

TL;DR: 作者指出Spavieri等人的主张存在循环论证的逻辑谬误，该主张声称Wang等人的干涉实验推翻了狭义相对论并证明了同时性的绝对性。


<details>
  <summary>Details</summary>
Motivation: 回应Spavieri等人对Wang等人干涉实验的错误解读，该解读声称实验推翻了狭义相对论并证明了同时性的绝对性，作者旨在揭示这种主张的逻辑缺陷。

Method: 通过逻辑分析指出Spavieri等人的论证存在循环论证（circular reasoning）的逻辑谬误，即他们的结论已经隐含在前提假设中。

Result: 证明Spavieri等人的主张在逻辑上无效，不能作为推翻狭义相对论或证明同时性绝对性的有效论据。

Conclusion: Spavieri等人对Wang实验的解读存在根本性的逻辑错误，因此不能支持"狭义相对论被推翻"或"同时性是绝对的"这样的结论。
...
### [18] [Tetrads in SU(N) Yang-Mills geometrodynamics](https://arxiv.org/abs/2603.18106)
*Alcides Garat*

Main category: gr-qc

TL;DR: 该论文研究如何将SU(N)对称性耦合到四维弯曲洛伦兹时空的引力场中，扩展了先前SU(3)对称性的工作，旨在实现大场统一。


<details>
  <summary>Details</summary>
Motivation: 随着粒子物理发展，需要纳入新的对称性（如SU(4)），因此需要研究如何将更一般的SU(N)对称性耦合到引力场中，实现对称性群与引力场的统一描述。

Method: 引入新的标架（tetrads），类似于SU(3)×SU(2)×U(1)情况的方法，利用标架的突出性质构建耦合。证明关于局部对称性规范群与局部标架变换群张量积的同构性质的新定理。

Result: 建立了SU(N)对称性与引力场的耦合框架，证明了相关同构定理，为四维弯曲洛伦兹时空中的大场统一提供了理论基础。

Conclusion: 成功扩展了SU(3)对称性耦合到引力场的方法到更一般的SU(N)情况，通过新的标架构造和同构定理，为实现大场统一提供了数学框架。
...
### [19] [The Steep Price of No Hair in Thiemann Regularized Loop Quantum Cosmology](https://arxiv.org/abs/2603.18175)
*Meysam Motaharfar,Parampreet Singh*

Main category: gr-qc

TL;DR: Thiemann正则化的圈量子宇宙学中，各向异性Bianchi-I时空会出现普朗克尺度的de Sitter相，自然抑制各向异性剪切并消除宇宙毛发，但代价是宇宙永远不会变得真正经典，且这种各向同性化机制是非普适的。


<details>
  <summary>Details</summary>
Motivation: 探索Thiemann正则化圈量子宇宙学中各向异性Bianchi-I时空的普朗克尺度物理，特别是各向异性剪切阻尼机制的起源和局限性，澄清和重新解释Gan等人的相关研究结果。

Method: 采用Thiemann正则化的圈量子宇宙学方法，研究Bianchi-I各向异性时空的量子化，分析量子引力效应对各向异性剪切的影响机制。

Result: 各向异性模型中存在与各向同性模型类似的涌现de Sitter相，能自然抑制各向异性剪切并消除宇宙毛发，但导致宇宙永远不会变得真正经典，且这种各向同性化机制是非普适的。

Conclusion: Thiemann正则化圈量子宇宙学中的各向异性剪切阻尼机制源于量子引力效应，能独立于初始条件和物质内容抑制各向异性，但该机制存在局限性：宇宙无法变得真正经典，且机制本身是非普适的。
...
### [20] [Robinson-Trautman spacetimes in (2+1) dimensions](https://arxiv.org/abs/2603.18209)
*Alberto Saa*

Main category: gr-qc

TL;DR: 本文提出了一种(2+1)维时空中的Robinson-Trautman演化，保留了四维情况的关键结构特征，通过引入四阶长度保持演化方程，数值解表明初始数据会弛豫到稳态区域，为低维引力中的耗散动力学提供了一个简单玩具模型。


<details>
  <summary>Details</summary>
Motivation: 研究动机是构建一个保留四维Robinson-Trautman演化关键特征的(2+1)维时空模型，为低维引力中的耗散动力学提供一个简单玩具模型，特别是研究由零辐射驱动的动力学过程。

Method: 方法包括：1）选择具有宇宙学常数的非稳态几何；2）引入四阶长度保持演化方程控制函数P(u,φ)；3）数值求解该演化方程；4）分析稳态配置对应负宇宙学常数下的boosted BTZ黑洞。

Result: 数值解强烈支持一般正则初始数据P(0,φ)会弛豫到稳态区域，稳态配置对应负宇宙学常数下的boosted BTZ黑洞，系统表现出与真正引力辐射现象相似的结构特征。

Conclusion: 该研究成功构建了一个简单的(2+1)维玩具模型，用于研究由零辐射驱动的耗散动力学，具有与四维引力辐射现象相似的结构特征，为理解低维引力中的类似现象提供了新视角。
...
### [21] [Holographic Quantum Foam: Theoretical Underpinnings and Observational Evidence](https://arxiv.org/abs/2603.18277)
*Eric Steinbring,Y. Jack Ng*

Main category: gr-qc

TL;DR: 该论文提出全息量子泡沫（HQF）理论，认为时空因量子涨落而呈泡沫状，并预测宇宙中存在暗物质暗能量。通过分析GRB221009A的观测数据，发现其与HQF理论预测的模糊效应一致。


<details>
  <summary>Details</summary>
Motivation: 验证全息量子泡沫（HQF）理论是否与实验观测一致，特别是通过观测遥远点源（如类星体和伽马射线暴）的模糊效应来探测普朗克尺度的物理效应。

Method: 分析GRB221009A这一极其明亮的高能伽马射线暴的多波段观测数据，计算HQF理论预测的点扩散函数（PSF）展宽效应，并与实际望远镜观测结果进行比较。

Result: GRB221009A从光学/近红外到X射线乃至最高能伽马射线的全谱观测数据，均与HQF理论预测的模糊效应一致，为HQF理论提供了观测支持。

Conclusion: 全息量子泡沫理论预测的暗物质暗能量存在，且其量子遵循无限统计而非传统费米或玻色统计。通过GRB221009A的观测数据，首次获得了与HQF理论一致的实验证据，表明该理论可能正确描述了普朗克尺度的时空量子涨落。
...
### [22] [Contrasting behaviour of two spherically symmetric perfect fluids near a weak null singularity in a spherically symmetric black hole](https://arxiv.org/abs/2603.18340)
*Raya V. Mancheva*

Main category: gr-qc

TL;DR: 对比两种物质模型（尘埃和刚体流体）在弱零奇点附近的物理行为：尘埃密度保持有界，而刚体流体密度发散且速度趋于零奇点切向的类光向量。


<details>
  <summary>Details</summary>
Motivation: 研究不同物质模型在弱零奇点附近的物理行为差异，检验强宇宙监督猜想在特定时空背景下的适用性。

Method: 1. 对尘埃模型：求解柯西问题，分析尘埃落入弱零奇点的运动，证明无壳交叉、速度保持类时、能量密度有界。2. 对刚体流体模型：通过特征初值问题，将流体速度与能量密度关联到满足齐次线性波动方程的标量场，分析奇点附近行为。

Result: 尘埃模型：速度流无壳交叉、速度保持类时、能量密度在奇点处有界。刚体流体模型：能量密度在奇点处发散，流入速度分量发散而流出分量趋于零，速度向量趋于与奇异超曲面相切的流入类光向量。

Conclusion: 不同物质模型在弱零奇点附近表现出截然不同的物理行为：尘埃保持良好行为而刚体流体出现发散，这表明物质模型的选择对奇点附近物理有重要影响。
...
### [23] [Black Hole--Entropy Container or Creator](https://arxiv.org/abs/2603.18374)
*William G Unruh*
...
<details>
  <summary>Details</summary>
Motivation: 挑战主流观点：传统认为黑洞拥有熵，蒸发时熵被发射并减少。本文质疑黑洞是否真正拥有熵，还是仅仅在过程中创造熵。

Method: 使用线性放大器模型，该模型本身没有熵但在运行过程中发射熵。将此模型与黑洞行为类比，分析熵的产生机制。

Result: 黑洞不拥有熵，而是在蒸发过程中创造并发射熵。总发射熵与传统表达式一致，与黑洞质量平方成正比。

Conclusion: 黑洞不拥有熵，而是创造熵。这一观点挑战了传统黑洞热力学，为理解黑洞熵的本质提供了新视角。
...
### [24] [Bosonic and fermionic mutual information of N-partite systems in dilaton black hole background](https://arxiv.org/abs/2603.18439)
*Xiao-Wei Teng,Rui-Yang Xu,Hui-Chen Yang,Shu-Min Wu*

Main category: gr-qc

TL;DR: 研究GHS膨胀子黑洞背景下自由玻色子和费米子场的N部量子关联，分析霍金效应对GHZ和W态多部互信息的影响，发现费米子互信息大于玻色子，但费米子相干相对熵小于玻色子。


<details>
  <summary>Details</summary>
Motivation: 研究弯曲时空（特别是GHS膨胀子黑洞）中的多部量子关联，探索霍金效应如何影响不同粒子种类（玻色子/费米子）和量子态（GHZ/W态）的量子信息特性，为相对论量子信息任务优化量子资源选择提供理论依据。

Method: 分析GHS膨胀子黑洞背景下自由玻色子和费米子场的N部态，聚焦GHZ和W态，让一个观察者悬停在事件视界附近，其余观察者位于渐近平坦区域，通过追踪事件视界内不可访问模式，推导膨胀子时空中N部互信息的解析表达式。

Result: 费米子互信息在膨胀子黑洞影响下大于玻色子互信息，但费米子相干相对熵小于玻色子相干相对熵；GHZ态的互信息始终大于W态，而GHZ态的相干相对熵在弯曲时空中小于W态。

Conclusion: 量子资源的选择应根据粒子种类和态结构进行定制，以优化相对论量子信息任务的操作效率，费米子和GHZ态在某些量子信息度量上表现出优势，但在其他度量上则相反。
...
### [25] [Scalarization of charged Taub-NUT black hole and the entropy bound](https://arxiv.org/abs/2603.18562)
*Lei Zhang,Hai-Shan Liu*

Main category: gr-qc

TL;DR: 带电Taub-NUT黑洞在爱因斯坦-麦克斯韦-标量-高斯-博内引力理论中会发生自发标量化，产生带毛的带电Taub-NUT黑洞解，其熵具有两个普适特征：总是大于无标量解，且在分岔点达到局部最大值。


<details>
  <summary>Details</summary>
Motivation: 研究带电Taub-NUT黑洞在扩展引力理论中的自发标量化现象，探索标量化黑洞解的性质，特别是熵的特征。

Method: 在爱因斯坦-麦克斯韦-标量-高斯-博内引力理论框架下，通过选择合适的耦合函数，使理论允许解析的带电Taub-NUT几何作为解。分析标量自由背景在特定参数范围内的不稳定性，研究由此分岔出的带毛带电Taub-NUT黑洞解。

Result: 发现带电Taub-NUT黑洞会发生自发标量化，产生由电荷和NUT参数张成的二维参数空间中的带毛黑洞解。这些解的熵具有两个普适特征：1）带毛黑洞的熵严格大于其标量自由对应物；2）熵在分岔点精确达到局部最大值。当电荷固定时，该最大熵值在特定质量参数范围内保持普适。

Conclusion: 带电Taub-NUT黑洞在扩展引力理论中会发生自发标量化，产生的带毛黑洞解具有独特的熵特征，这些普适性质为理解标量化黑洞的热力学行为提供了新见解。
...
### [26] [Exact pp-wave solutions in shift-symmetric higher-order scalar-tensor theories](https://arxiv.org/abs/2603.18794)
*Masato Minamitsuji*

Main category: gr-qc

TL;DR: 论文研究了在平移对称二次高阶标量-张量理论中精确的平面波引力波解，发现了与引力波共存但不影响时空几何的"隐形"标量场配置。


<details>
  <summary>Details</summary>
Motivation: 研究在修正引力理论框架下，超越线性近似的完全非线性辐射时空解，探索标量场与引力波的非线性相互作用。

Method: 采用标量场依赖于横向坐标和推迟零坐标的假设，保持标量场动能项恒定，在特定耦合条件下将引力场方程简化为二维拉普拉斯方程。

Result: 发现了"隐形平面波"解，其中非平凡标量场与引力波共存但不影响时空几何；这些解与DHOST理论的退化条件兼容且满足观测约束；在共形变换下隐形特性一般会消失。

Conclusion: 平面波解在可行的DHOST框架中具有鲁棒性，为探测修正引力中的非线性效应提供了有用工具，但隐形特性对特定场配置敏感。
...
### [27] [Thermodynamics of Kerr-Bertotti-Robinson black hole](https://arxiv.org/abs/2603.18821)
*Li Hu,Rong-Gen Cai,Shao-Jiang Wang*

Main category: gr-qc

TL;DR: 该论文研究了Kerr-Bertotti-Robinson黑洞的热力学性质，这是一个描述旋转黑洞浸没在外电磁场中的爱因斯坦-麦克斯韦理论精确解。通过采用Christodoulou-Ruffini质量关系作为守恒质量的热力学定义，解决了标准可积性方法无法获得守恒质量的问题。


<details>
  <summary>Details</summary>
Motivation: 研究Kerr-Bertotti-Robinson黑洞的热力学性质，特别是解决由于非平凡渐近均匀外电磁场导致标准可积性方法无法获得守恒质量的问题。

Method: 采用Christodoulou-Ruffini质量关系作为守恒质量的热力学定义，识别相关的生成元，从而固定守恒质量定义的模糊性并构建热力学势。

Result: 成功定义了守恒质量和相关热力学量，这些量自然满足黑洞热力学第一定律和Smarr公式。

Conclusion: 通过Christodoulou-Ruffini质量关系方法，成功解决了Kerr-Bertotti-Robinson黑洞在非平凡外电磁场背景下的热力学描述问题，建立了完整的热力学框架。
...
### [28] [Analytic Expressions for Quasinormal Modes of a Regular Black Hole Sourced by a Dehnen-Type Halo](https://arxiv.org/abs/2603.18887)
*Zainab Malik*

Main category: gr-qc

TL;DR: 推导了Dehnen型暗物质晕中渐近平坦黑洞引力准正则模的紧凑解析表达式


<details>
  <summary>Details</summary>
Motivation: 研究超大质量黑洞在星系环境中（由Dehnen型暗物质晕支持）的引力扰动特性，特别是准正则模的解析描述

Method: 使用超越eikonal近似的展开方法，推导轴向引力扰动的解析表达式，将扰动分为"上"和"下"两个非等谱通道

Result: 获得了相对紧凑且精确的解析表达式，描述了黑洞在暗物质晕中的准正则模特性，发现两个扰动通道具有不同的谱特性

Conclusion: 成功推导了Dehnen型暗物质晕中黑洞引力准正则模的解析表达式，揭示了轴向引力扰动分裂为两个非等谱通道的重要特性
...
### [29] [Observational Signatures of Rotating Ayón-Beato-García Black Holes: Shadows, Accretion Disks and Images](https://arxiv.org/abs/2603.18963)
*Zhenglong Ban,Meng Chen,Rong-Jia Yang*

Main category: gr-qc

TL;DR: 研究旋转Ayón-Beato-García黑洞的阴影、吸积盘和观测图像，发现电荷参数ζ增加会减小阴影尺寸，在近极端旋转下形成"D"形阴影，并通过与EHT观测对比约束了参数空间。


<details>
  <summary>Details</summary>
Motivation: 研究带电旋转黑洞的观测特征，特别是阴影形态和吸积盘图像，以理解黑洞参数（质量、自旋、电荷）如何影响观测现象，并与实际EHT观测数据对比来约束理论参数。

Method: 分析旋转Ayón-Beato-García黑洞的阴影和吸积盘，考虑粒子在ISCO内外不同动力学，计算直接和透镜化图像，分析红移分布，并与EHT对M87*和Sgr A*的观测数据对比。

Result: 阴影尺寸随电荷ζ增加而减小，近极端旋转时形成"D"形阴影；图像不对称性和内阴影畸变受(a,ζ)和观测倾角显著影响；高倾角下直接和透镜图像分离形成帽状结构；通过与EHT观测对比得到参数约束0.132811M < ζ < 0.213607M。

Conclusion: 旋转Ayón-Beato-García黑洞的观测特征对电荷参数敏感，近极端旋转产生独特"D"形阴影，与EHT观测对比为黑洞电荷参数提供了约束，支持该理论模型与观测数据的一致性。
...
### [30] [Revisiting $f(T)$ Teleparallel Gravity with a Parametrized Hubble Parameter and Observational Constraints](https://arxiv.org/abs/2603.18971)
*Khomesh R. Patle,G. P. Singh*

Main category: gr-qc

TL;DR: 在f(T)引力框架下研究宇宙加速膨胀的动力学行为，通过假设特定哈勃参数形式建立两个宇宙学模型，利用贝叶斯统计方法拟合观测数据，分析宇宙学参数演化、能量条件及状态诊断。


<details>
  <summary>Details</summary>
Motivation: 研究f(T)引力理论下宇宙加速膨胀的动力学行为，探索修正引力理论对宇宙演化的影响，通过建立具体模型来理解暗能量的性质。

Method: 假设特定形式的哈勃参数H(z)，推导出两个不同的宇宙学模型；使用贝叶斯统计技术和χ²最小化方法，结合宇宙计时器(CC)数据集和联合(CC+Pantheon)数据集拟合模型参数；分析减速参数、能量密度、压强、暗能量状态方程等宇宙学参数的演化。

Result: 确定了两个模型参数的中间值；分析了减速参数从减速到加速的转变；研究了能量密度、压强和暗能量状态方程的演化；检验了能量条件的有效性；探讨了状态诊断器的性质；估算了当前宇宙的年龄。

Conclusion: 提出的f(T)引力模型能够描述宇宙加速膨胀，与观测数据一致；模型参数通过统计方法得到约束；能量条件和状态诊断分析为理解暗能量性质提供了见解；宇宙年龄估计与当前观测相符。
...
TL;DR: 对GWTC-4.0中91个引力波事件进行广义相对论一致性测试，未发现偏离广义相对论的证据


<details>
  <summary>Details</summary>
Motivation: 随着LIGO-Virgo-KAGRA探测器灵敏度提升，需要利用更多引力波信号在强场动力学区域对广义相对论进行更精确的测试

Method: 使用GWTC-4.0目录中91个置信信号（包括42个O4a事件），进行四种一致性测试：残差与噪声一致性、低频与高频波形推断的最终质量和自旋一致性、次主导引力波多极矩振幅一致性、非广义相对论偏振模式一致性

Result: 所有测试均未发现偏离广义相对论的证据：残差与噪声一致，低频与高频推断的最终参数一致，次主导多极矩振幅符合广义相对论预测，未发现非广义相对论偏振模式

Conclusion: 广义相对论（无新物理）仍与这些引力波事件一致，三篇论文中的另外两篇也发现总体与真空广义相对论一致，90%以上事件在90%置信水平下与广义相对论一致
...
### [32] [GWTC-4.0: Tests of General Relativity. II. Parameterized Tests](https://arxiv.org/abs/2603.19020)
...
TL;DR: 对GWTC-4.0中91个引力波事件的参数化广义相对论测试表明，没有发现超出广义相对论的物理证据，对后牛顿参数偏差的约束提高了1.2-5.5倍，引力子质量上限更新为1.92×10⁻²³ eV/c²。


<details>
  <summary>Details</summary>
Motivation: 通过分析LIGO Virgo KAGRA探测器第四观测运行期(O4a)及之前的引力波事件，对广义相对论进行参数化测试，检验是否存在超出广义相对论的物理效应，包括后牛顿参数修正、自旋诱导四极矩偏差、色散或双折射传播效应等。

Method: 使用GWTC-4.0中42个O4a高置信度事件（假警报率≤10⁻³ yr⁻¹）加上之前观测期的49个类似事件，共91个事件。采用八种参数化测试方法，包括后牛顿参数修正、自旋诱导四极矩测试、传播效应测试等，约束与广义相对论预测值的偏差。

Result: 没有发现超出广义相对论的物理证据，90%以上的事件在90%置信区间内包含零偏差结果。对后牛顿参数偏差的约束提高了1.2-5.5倍，引力子质量上限更新为1.92×10⁻²³ eV/c²。没有发现自旋诱导四极矩与克尔黑洞不同的证据，也没有发现视线加速度的证据。

Conclusion: 广义相对论在强场引力测试中仍然成立，许多对可能偏差的约束是目前最好的结果。虽然某些事件存在可能的系统误差，但统计上并不意外，符合噪声预期。
...
### [33] [GWTC-4.0: Tests of General Relativity. III. Tests of the Remnants](https://arxiv.org/abs/2603.19021)
...
TL;DR: 对GWTC-4.0引力波信号进行广义相对论测试，重点分析42个双星并合事件的残余信号，发现总体与GR一致，未发现合并后回波证据


<details>
  <summary>Details</summary>
Motivation: 利用LIGO-Virgo-KAGRA第四观测期的新数据，对引力波信号中的并合残余进行更全面的广义相对论检验，特别是测试黑洞环降和可能的回波信号

Method: 分析GWTC-4.0中42个高置信度事件，采用七种测试方法：三种测试环降与Kerr黑洞准正规模谱的一致性（两种时域分析，一种频域分析），四种测试可能的合并后回波

Result: 总体与GR一致，一个分析发现GR预测位于98.6%置信区间边界，但考虑有限事件数的方差后显著性降低。未发现合并后回波的证据，特别是加入GW250114事件后显著性降至92.2%

Conclusion: 引力波并合残余与广义相对论预测基本一致，没有发现偏离GR的强有力证据，也没有检测到理论预测外的合并后回波信号
...
### [34] [Thermodynamic Analysis of Charged AdS Black Holes with Cloud of Strings in Einstein-Bumblebee Gravity via Tsallis Entropy](https://arxiv.org/abs/2603.19034)
*Faizuddin Ahmed,Edilberto O. Silva*

Main category: gr-qc

TL;DR: 研究弦云和洛伦兹破缺参数如何改变带电AdS黑洞的热力学性质，包括相变、临界行为、Joule-Thomson膨胀和非广延熵效应。


<details>
  <summary>Details</summary>
Motivation: 探索在弦云环境和Bumblebee引力框架下，洛伦兹对称性破缺如何影响黑洞的热力学结构，特别是相变行为和临界现象。

Method: 在扩展相空间描述中，分析弦云参数α和洛伦兹破缺参数ℓ对黑洞视界结构、温度、自由能、比热和临界行为的影响，并研究Joule-Thomson膨胀和非广延Tsallis熵框架。

Result: 系统表现出Van der Waals型的小-大黑洞相变，临界量被弦云和Bumblebee背景修正，洛伦兹破缺改变了普适比。Joule-Thomson膨胀的加热-冷却边界被变形参数移动，非广延参数δ显著改变温度分布、稳定性窗口和霍金辐射稀疏性。

Conclusion: 弦云、洛伦兹破缺和非广延熵的联合效应产生了比标准Reissner-Nordström-AdS黑洞更丰富的热力学结构，揭示了引力理论中对称性破缺和非广延统计的重要影响。
...
### [35] [Scalar field in Bianchi type-I cosmology with Lyra's geometry](https://arxiv.org/abs/2603.19072)
*Evgeny Petuhov,Bijan Saha*

Main category: gr-qc

TL;DR: 研究在Lyra几何框架下的Bianchi I型宇宙学模型中，标量场在宇宙演化中的作用，分析了Lyra参数的动力限制和能量-动量张量守恒的破坏。


<details>
  <summary>Details</summary>
Motivation: 先前研究已在各向异性和各向同性宇宙学模型中探索了非线性旋量场，本研究旨在在Lyra几何框架下研究标量场在宇宙演化中的作用，特别是Lyra参数的动力限制和能量-动量张量守恒的破坏问题。

Method: 采用Lyra几何框架下的Bianchi I型宇宙学模型，分析标量场在其中的演化，研究Lyra参数的动力限制条件，并探讨能量-动量张量守恒的破坏机制。

Result: 研究表明，在考虑的情况下，Lyra参数的行为对应于早期宇宙中的相对影响，而在当前宇宙中Lyra几何的影响消失。这意味着Lyra几何在宇宙演化过程中具有时间依赖性。

Conclusion: Lyra几何在早期宇宙演化中起重要作用，但在当前宇宙中其影响可以忽略，这为理解宇宙演化的不同阶段提供了新的理论框架。
...
### [36] [Black hole superradiance in Poincaré gauge theory](https://arxiv.org/abs/2603.19140)
*Sebastian Bahamonde,Jorge Gigante Valcarcel*

Main category: gr-qc

TL;DR: 论文研究了在Poincaré规范理论框架下，带有挠率的黑洞超辐射现象，发现挠率使狄拉克费米子能够通过手征不对称从旋转黑洞中提取能量，同时保持泡利不相容原理。


<details>
  <summary>Details</summary>
Motivation: 传统广义相对论方法无法解释狄拉克费米子如何从旋转黑洞中提取能量而不违反泡利不相容原理，因此需要研究包含挠率的时空几何如何影响黑洞超辐射现象。

Method: 在Poincaré规范理论框架下，将挠率纳入时空几何，研究旋转黑洞背景中狄拉克费米子的行为，特别是通过手征不对称机制分析能量提取过程。

Result: 研究表明，与经典广义相对论方法不同，包含挠率的时空几何使得狄拉克费米子能够通过手征不对称从旋转黑洞中提取能量，同时保持泡利不相容原理。

Conclusion: 挠率在黑洞超辐射中起着关键作用，为狄拉克费米子从旋转黑洞中提取能量提供了新机制，这一发现扩展了我们对引力理论中费米子行为的理解。
