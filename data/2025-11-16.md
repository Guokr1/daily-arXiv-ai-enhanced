...
### [1] [An ASME-Compliant Helium-4 Evaporation Refrigerator for the SpinQuest Experiment](https://arxiv.org/abs/2511.09689)
*Jordan D. Roberts,Vibodha Bandara,Kenichi Nakano,Dustin Keller*

Main category: physics.ins-det

TL;DR: 本文介绍了为费米实验室SpinQuest实验开发的1K液氦-4蒸发制冷机的设计、安全基础和调试结果，这是费米实验室首个在固定靶散射实验中运行的高功率氦蒸发制冷系统。


<details>
  <summary>Details</summary>
Motivation: 为SpinQuest实验开发符合费米实验室安全要求的1K液氦蒸发制冷系统，填补该领域在费米实验室合规性方面的空白。

Method: 基于ASME B31.3和ASME锅炉压力容器规范进行设计，确保压力边界完整性和超压保护，并符合FESHM相关章节要求。针对高辐射环境采用远程仪器布置（距离低温恒温器约20米），并设计了适应瞬态热负荷的泄放系统。

Result: 2024年7月的调试数据确认系统满足所有热性能和安全目标。

Conclusion: 成功开发并验证了符合费米实验室安全标准的1K液氦蒸发制冷系统，为高辐射环境下的固定靶实验提供了可靠解决方案。
...
### [2] [High-throughput Parasitic-independent Probe Thermal Resistance Calibration for Robust Thermal Mapping with Scanning Thermal Micrsocopy](https://arxiv.org/abs/2511.09960)
...
<details>
  <summary>Details</summary>
Motivation: 纳米结构材料在热管理中具有尺寸依赖性，需要高空间分辨率的热传输表征技术。传统扫描热显微镜在信号转换和表面敏感性方面存在挑战，限制了其在纳米尺度热传输研究中的应用。

Method: 开发了基于电路探针热阻校准技术，独立于寄生热传输路径，通过测量SThM探针耗散的热量和尖端温度变化来提取薄膜热阻。使用有限元建模校准方法将热阻映射转换为热导率。

Result: 在15纳米厚的铝薄膜上实现了亚100纳米空间分辨率的热阻映射，获得的有效热导率为2.87±0.18 W m^-1 K^-1，与超薄铝热传输理论框架吻合良好。

Conclusion: 该校准技术使SThM成为纳米尺度热传输和缺陷介导热性能调制的可靠表征标准，为热界面材料设计提供了重要工具。
...
### [3] [Autonomous X-ray Fluorescence Mapping of Chemically Heterogeneous Systems via a Correlative Feature Detection Framework](https://arxiv.org/abs/2511.09975)
*Carlos Deleon,Dmitri Gavrilov,Peggy ODay,Ajith Pattammattel*

Main category: physics.ins-det

TL;DR: X-AutoMap是一个用于自主X射线荧光映射的模块化框架，通过相关特征检测策略实现化学信息引导的区域目标定位，显著减少高分辨率采集时间。


<details>
  <summary>Details</summary>
Motivation: 传统XRF映射方法耗时且依赖人工干预，需要开发能够自动识别化学成分相关区域并优化扫描效率的系统。

Method: 集成经典计算机视觉和基于规则的逻辑，通过多元素图的空间关系识别特征，与Bluesky控制基础设施紧密集成实现实时闭环扫描编排，支持交互式和自主模式。

Result: 应用于城市PM2.5样品，将高分辨率采集时间从44小时减少到约10小时，成功识别出完全混合、部分重叠和空间分离的多元素结构等不同颗粒类型。

Conclusion: X-AutoMap能够以最小用户干预隔离化学相关特征，为同步辐射光束线提供可扩展的智能成像工作流程基础，未来将整合机器学习和概率采样进一步提高性能。
...
### [4] [Temperature Dependence of the Time Resolution in a SiPM-Readout Plastic Scintillator for Cosmic-Ray Applications](https://arxiv.org/abs/2511.10041)
*José Reyes Castillo,Saúl Aguilar Salazar,Diego Mauricio Gomez Coral*

Main category: physics.ins-det

TL;DR: 研究了温度变化对基于塑料闪烁体和SiPM的时间飞行探测器性能的影响，在-20到20°C范围内测试了时间分辨率稳定性


<details>
  <summary>Details</summary>
Motivation: 气球和空间宇宙射线实验中的塑料闪烁体探测器在飞行中会经历温度变化，需要了解温度对探测器时间分辨率的影响

Method: 构建了BC418塑料闪烁体棒与Onsemi MICROFC-30050 SiPMs组成的TOF原型，在-20到20°C可控热环境中测试，使用锶-90源电子束和专用差分前置放大器

Result: 在3V过电压下实现了160ps的最小时间分辨率，该分辨率在测试温度范围内保持稳定，且在闪烁体棒上分布均匀

Conclusion: 该探测器系统在宽温度范围内表现出稳定的时间分辨率性能，适用于气球和空间宇宙射线实验
...
### [5] [Time resolution of the ALICE Time-Of-Flight detector with the first Run 3 pp collisions at ${\bf \sqrt{\textit{s}} = 13.6}$ TeV](https://arxiv.org/abs/2511.10311)
*ALICE Collaboration*

Main category: physics.ins-det

TL;DR: ALICE TOF探测器在LHC运行3期间进行了升级，实现了优于80 ps的时间分辨率，提升了粒子识别能力


<details>
  <summary>Details</summary>
Motivation: 为了适应LHC运行3中Pb-Pb碰撞50 kHz的相互作用率，需要对TOF探测器进行升级以支持连续读取方案

Method: 升级了TOF探测器电子设备，重写了数据处理算法，使用2022年13.6 TeV pp碰撞数据，通过两种独立方法评估时间分辨率

Result: 两种方法均得到一致结果，时间分辨率优于80 ps，性能已很优秀但预计通过进一步调试和校准还能提升

Conclusion: 升级后的TOF探测器在运行3中表现出色，时间分辨率显著改善，为ALICE的粒子识别能力提供了重要支撑
...
### [6] [Development of the CEPC analog hadron calorimeter prototype](https://arxiv.org/abs/2511.10377)
...
TL;DR: CEPC AHCAL原型机通过电子测试、宇宙射线测试和三次束流测试验证了其可行性，目前正在分析测试数据以优化探测器设计。


<details>
  <summary>Details</summary>
Motivation: 为CEPC探测器开发模拟强子量能器(AHCAL)选项，以支持粒子流算法(PFA)概念，实现精确的希格斯玻色子性质测量。

Method: 采用40层钢吸收体和闪烁体瓦片与硅光电倍增管耦合的夹层结构，通过电子测试、宇宙射线测试和三次束流测试验证原型机性能。

Result: 原型机成功完成所有测试，测试数据正在分析中，预计将深化对强子簇射的理解。

Conclusion: AHCAL原型机测试验证了该方案的可行性，为CEPC探测器设计优化提供了重要数据支持。
...
### [7] [Edge Machine Learning for Cluster Counting in Next-Generation Drift Chambers](https://arxiv.org/abs/2511.10540)
*Deniz Yilmaz,Liangyu Wu,Julia Gonski*

Main category: physics.ins-det

TL;DR: 该论文提出了在漂移室实时读出系统中使用机器学习算法进行簇计数的方法，相比传统基于导数的方法能实现更好的π介子-κ介子分离，且FPGA实现能满足未来希格斯工厂的实时操作要求。


<details>
  <summary>Details</summary>
Motivation: 未来希格斯工厂等对撞机需要更高粒度和簇计数能力以实现粒子识别，这给数据处理带来新挑战。在读出单元级别进行边缘机器学习可以显著减少高粒度漂移室的离探测器数据率。

Method: 开发了用于未来漂移室实时读出的机器学习簇计数算法，并将这些算法合成为FPGA资源以实现低延迟操作。

Result: 这些机器学习算法在可实现的π介子-κ介子分离方面优于传统的基于导数的技术，FPGA实现能够达到与未来希格斯工厂场景实时操作一致的延迟。

Conclusion: 该研究不仅推动了未来对撞机探测器的研发，也促进了高能物理中基于硬件的边缘机器学习应用。
...
### [8] [Reduced Variability in Threshold Switches Using Heterostructures of SiO${_x}$ and Vertically Aligned MoS${_2}$](https://arxiv.org/abs/2511.09561)
...
TL;DR: 研究了垂直排列二维MoS2/SiOx异质结构中基于银丝形成的阈值开关机制，发现vdW间隙引导的受限离子迁移能够形成超薄种子丝，从而提高开关性能和可靠性。


<details>
  <summary>Details</summary>
Motivation: 二维材料的vdW间隙可以调控离子迁移和丝生长，但基本开关机制及其对电阻开关的贡献尚未充分理解，需要研究垂直排列二维材料异质结构中的开关机制。

Method: 通过硫化合成了垂直排列二维MoS2，构建了SiOx/VAMoS2异质结构器件，与纯SiOx器件对比，分析了阈值开关特性，并建立了基于物理的可变性感知模型。

Result: SiOx/VAMoS2器件表现出更高的开关阈值和保持电压（约0.4V）、更快的开关时间（356ns@4V脉冲）和更低的循环间导通电流变异性（3.0%）。

Conclusion: SiOx/VAMoS2异质结构通过vdW间隙中受限的Ag离子迁移形成超薄种子丝，为垂直器件架构中的可靠阈值开关提供了有前景的概念，适用于新兴存储器和神经形态计算。
...
### [9] [Diamond-based sensing of stray fields from the bulk of thin-film magnets via nano-indentation](https://arxiv.org/abs/2511.10176)
...
<details>
  <summary>Details</summary>
Motivation: 测量薄膜或二维材料内部的磁化强度对于理解其固有性质很重要，但由于内部杂散场微弱或消失，直接测量方法受到限制。

Method: 采用纳米压痕技术诱导材料内部杂散场泄漏，并使用纳米金刚石磁强计在纳米尺度空间分辨率下测量这些杂散场。
...
### [10] [Bandwidth of Linear Classically Damped Systems with Application to Experimental Model Aircraft](https://arxiv.org/abs/2511.10379)
...
<details>
  <summary>Details</summary>
Motivation: 传统带宽概念在线性、单模态响应和轻阻尼条件下存在局限性，RMS带宽虽然克服了这些限制但缺乏物理意义解释。

Method: 推导了基于系统模态参数和初始模态能量分布的ARMS带宽，并开发了数据驱动的模态能量分布评估方法。

Result: ARMS带宽能够可靠准确地计算实际结构的耗散能力，适用于单/多自由度系统和实验模型飞机。

Conclusion: ARMS带宽为结构能量耗散能力提供了有效的单一度量指标，未来将研究非经典阻尼、时变参数和非线性的影响。
...
### [11] [Electron Heat Flux and Whistler Instability in the Earth's Magnetosheath](https://arxiv.org/abs/2511.10275)
...
TL;DR: 使用MMS卫星数据研究地球磁鞘中的电子热通量，发现热通量受磁鞘磁场控制，受太阳风上游条件影响，但不受局部磁鞘过程显著改变，且受哨声波不稳定性阈值限制。
...
Result: 热通量受磁鞘磁场围绕磁层弯曲的影响；受太阳风上游条件影响并随磁场强度增加；不受局部磁鞘过程显著改变；受哨声波不稳定性阈值限制。

Conclusion: 磁鞘电子热通量主要由上游太阳风条件和磁场构型决定，局部过程影响较小，且受等离子体不稳定性约束。
...
### [12] [Analysis of the TAIGA-HiSCORE Data Using the Latent Space of Autoencoders](https://arxiv.org/abs/2511.09655)
*Yu. Yu. Dubenskaya,S. P. Polyakov,A. P. Kryukov,A. P. Demichev,E. O. Gres,E. B. Postnikov,A. Yu. Razumov,P. A. Volchugov,D. P. Zhurov*

Main category: astro-ph.IM

TL;DR: 提出使用自编码器（AE）的潜在空间参数替代传统经验参数，用于TAIGA实验的广延大气簇射分析，以更好地保留物理信息并实现多模态数据集成。


<details>
  <summary>Details</summary>
Motivation: TAIGA实验的多探测器系统（IACTs和TAIGA-HiSCORE）信号物理性质不同，直接数据融合不可行，传统经验参数可能丢失重要信息。

Method: 使用自编码器从实验数据中提取潜在空间参数，再用单独的人工神经网络从潜在空间重建EAS初级粒子参数。

Result: 基于TAIGA-HiSCORE的蒙特卡洛模拟数据，使用AE潜在空间能够以满意精度重建初级粒子能量，并分析了潜在空间维度对精度的影响。

Conclusion: AE潜在空间参数方法能够有效重建EAS初级粒子能量，为异构探测器数据集成提供了潜在解决方案。
...
### [13] [Analyzing Exoplanet Transits Observed with the WFC3/UVIS G280 Grism](https://arxiv.org/abs/2511.09694)
*Munazza K. Alam,Frederick Dauphin,Amanda Pagul*

Main category: astro-ph.IM

TL;DR: 介绍了一个用于处理WFC3/UVIS G280光栅系外行星凌星观测数据的Jupyter笔记本，展示了从校准光谱到可拟合的凌星光变曲线的完整工作流程
...
Method: 使用Jupyter笔记本展示数据处理流程，包括背景扣除、宇宙线修正、光谱轨迹拟合、光谱提取和光变曲线生成等步骤

Result: 生成了原始的宽带和光谱光变曲线，这些数据可以输入到公开的光变曲线拟合工具中提取行星透射光谱

Conclusion: 该笔记本为处理WFC3/UVIS G280光栅观测数据提供了实用的工作流程示例，有助于标准化系外行星凌星数据的分析过程
...
### [14] [Mock Observations for the CSST Mission: CPI-C -- Targets for High Contrast Imaging](https://arxiv.org/abs/2511.09862)
*Yi-Ming Zhu,Gang Zhao,Jiang-Pei Dou,Zhong-Hua Lv,Yi-Li Chen,Bo Ma,Zhao-Jun Yan,Jing Tang,Ran Li*

Main category: astro-ph.IM

TL;DR: CPISM是为中国空间站望远镜上的冷行星成像日冕仪开发的模拟程序，支持高对比度系外行星成像，通过模拟观测条件和仪器效应来优化目标选择和观测策略。


<details>
  <summary>Details</summary>
Motivation: 为CPI-C任务开发一个能够模拟观测条件和仪器效应的工具，以优化目标选择、观测策略，并为未来的系外行星发现和表征工作提供支持。

Method: 采用模块化设计，包括目标建模、成像模拟、观测效应、探测器响应和数据产品生成模块，能够灵活模拟不同的恒星和行星模型以及各种观测效应。

Result: 通过对亮星的模拟验证显示与理论预期高度一致，证实了程序的准确性。程序能够模拟仪器噪声、宇宙射线和其他观测效应。

Conclusion: CPISM是一个准确且灵活的工具，将增强CPI-C任务的观测规划和科学产出，为系外行星系统研究提供关键见解。
...
### [15] [Topmetal-L: A Low Noise Charge-Sensitive Pixel Sensor for POLAR2/LPD](https://arxiv.org/abs/2511.09863)
...
TL;DR: Topmetal-L是一款专为POLAR-2空间天文平台低能偏振探测器升级而开发的电荷敏感像素传感器，采用130nm CMOS工艺制造，具有356×512像素阵列，在2-10 keV能量范围内实现伽马暴X射线偏振测量。


<details>
  <summary>Details</summary>
Motivation: 为POLAR-2空间天文任务的低能偏振探测器开发高性能电荷敏感像素传感器，以满足伽马暴X射线偏振测量的科学需求，特别是需要大面阵、高帧率读出的气体偏振探测器。

Method: 采用130nm CMOS工艺制造Topmetal-L芯片，集成356×512像素阵列，像素间距45μm。每个像素包含26×26μm²电荷收集电极窗口，可同时输出能量和位置信息。提出哨兵扫描读出方案优化读out效率。

Result: 芯片输入动态范围0-4ke-，电荷-电压转换增益76.04μV/e-，等效噪声电荷22.8e-，敏感面积>3.6cm²，总功耗720mW。原型系统在5.90keV残余调制0.26%±0.45%，8.0keV偏振源调制因子66.67%±0.45%，5.4keV计数率饱和于15k counts/(cm²·s)。

Conclusion: Topmetal-L成功开发并验证了适用于POLAR-2低能偏振探测器的电荷敏感像素传感器，满足大面阵、高帧率读出的要求，为伽马暴X射线偏振测量提供了关键技术支撑。
...
### [16] [fiDrizzle-MU: A Fast Iterative Drizzle with Multiplicative Updates](https://arxiv.org/abs/2511.09881)
*Shen Zhang,Lei Wang,Huanyuan Shan,Ran Li,Xiaoyue Cao,Yunhao Gao*

Main category: astro-ph.IM

TL;DR: fiDrizzleMU算法通过迭代乘法更新进行曝光叠加，替代传统加法校正框架，在叠加图像中实现更好的抗锯齿和降噪效果。


<details>
  <summary>Details</summary>
Motivation: 传统加法校正框架在处理天文图像时存在局限性，无法有效恢复被重采样过程模糊的微弱和扩展结构。

Method: 采用迭代乘法更新方法进行曝光叠加，取代传统的加法校正框架。

Result: 在詹姆斯·韦伯太空望远镜数据应用中，该算法成功重建了被管道重采样过程显著模糊的引力透镜候选体。

Conclusion: fiDrizzleMU算法能够准确恢复高分辨率天文成像中的微弱和扩展结构。
...
### [17] [Ginnungagap -- a massively parallel cosmological initial conditions generator](https://arxiv.org/abs/2511.10353)
*Sergey Pilipenko,Gustavo Yepes,Stefan Gottlöber,Steffen Knollmann*

Main category: astro-ph.IM

TL;DR: Ginnungagap是一个完全并行的MPI+OpenMP代码，用于生成包含大量粒子的宇宙学模拟初始条件，支持均匀或可变分辨率模式。


<details>
  <summary>Details</summary>
Motivation: 为大规模宇宙学模拟提供高效、灵活的初始条件生成工具，支持不同分辨率需求的模拟类型。

Method: 开发了模块化的开源代码，包含独立工具集，支持完全随机或基于现有初始条件扩展分辨率的生成方式。

Result: 成功生成了不同类型模拟的初始条件，并通过测试验证了其有效性。

Conclusion: Ginnungagap是一个功能强大且灵活的宇宙学初始条件生成工具，适用于各种大规模模拟需求。
...
### [18] [A scalable and accurate framework for self-calibrating null depth retrieval using neural posterior estimation](https://arxiv.org/abs/2511.10455)
*Baoyi Zeng,Marc-Antoine Martinod,Denis Defrère*

Main category: astro-ph.IM

TL;DR: 该论文将机器学习与贝叶斯推断相结合，开发了基于神经后验估计(NPE)的模型，用于提高零干涉测量中零深度检索的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 零干涉测量中准确检索零深度至关重要，但由于各种噪声源、仪器缺陷和实际观测环境的复杂性，实现准确的零深度校准具有挑战性，需要能够高效处理不确定性的先进校准技术。

Method: 开发了基于神经后验估计(NPE)的模型，使用包含真实数据的模拟器来更好地表示特定条件，并在LBTI零干涉仪的合成数据和观测数据上进行测试评估。

Result: NPE模型成功展示了改进的效率，在真实观测数据上实现了低至10^{-4}量级的零深度检索精度，与传统方法性能相当，同时将数据检索时间减少到自校准方法所需时间的四分之一。

Conclusion: NPE模型为零干涉测量中的零深度校准提供了一个实用且可扩展的解决方案，在效率上相比现有方法有显著改进，具有更好的精度并可应用于其他干涉测量技术。
...
### [19] [A new regularization scheme for the wave function of the Universe in the Lorentzian path integral](https://arxiv.org/abs/2511.09621)
*Masaki Yamada*

Main category: gr-qc

TL;DR: 提出了一种新的洛伦兹路径积分调节器，通过引入初始宇宙大小的微小量子不确定性来解决条件收敛问题，得到具有抑制扰动的隧穿波函数。


<details>
  <summary>Details</summary>
Motivation: 洛伦兹路径积分用于宇宙波函数时存在条件收敛问题，Picard-Lefschetz方法会导致未抑制的扰动，需要寻找替代方案。

Method: 在超空间中引入调节器：通过窄高斯函数表示初始宇宙大小的delta函数，宽度σ→0，使路径积分绝对收敛。

Result: 对于任意σ>0，路径积分沿实轴绝对收敛，无需进入ImN>0区域，得到具有适当抑制扰动的隧穿波函数。

Conclusion: 纯洛伦兹路径积分可以产生具有抑制扰动的隧穿波函数，Hartle-Hawking波函数也可通过特定积分路径复现。
...
### [20] [Dark matter by design: $Q$-balls, neural networks and galaxy rotation curves](https://arxiv.org/abs/2511.09634)
*Alexandre M. Pombo,Lorenzo Pizzuti,Alessandra di Giacomo*

Main category: gr-qc

TL;DR: 本文研究了旋转标量Q球是否能再现观测到的星系旋转曲线，发现简化的Q球模型与SPARC星表数据吻合良好，并给出了标量场粒子质量约束。


<details>
  <summary>Details</summary>
Motivation: Q球作为非拓扑孤子，在角动量作用下呈现环状结构，与盘状星系结构相似，这启发了研究旋转Q球是否能解释星系旋转曲线。

Method: 使用混合数值框架，结合高精度伪谱方法和物理信息神经网络方法，构建静态和旋转Q球解，然后拟合SPARC星表中的星系旋转曲线数据。

Result: 简化模型与观测数据整体吻合良好，得出标量场粒子质量平均约束为m∼10^{-27} eV，与类似星系尺度孤子解一致。

Conclusion: 旋转标量Q球能较好地再现星系旋转曲线，支持其作为暗物质晕的候选者，并提供了标量场粒子质量的约束。
...
### [21] [Hybrid model for inspiral-merger-ringdown gravitational waveforms from comparable-mass, nonspinning binary black holes](https://arxiv.org/abs/2511.09646)
*Nur E. M. Rifat,David A. Nichols,Kent Yagi*

Main category: gr-qc

TL;DR: 本文提出了一种改进的混合近似方法，结合后牛顿理论和黑洞微扰理论来模拟双黑洞并合波形，通过使用有效单体方法优化边界轨迹，并调整势函数以匹配准正规模频率，实现了与数值相对论波形的高度一致性。


<details>
  <summary>Details</summary>
Motivation: 传统方法使用后牛顿理论和黑洞微扰理论分别模拟双黑洞波形的旋进和环降阶段，但并合阶段主要依赖数值相对论。本文旨在开发一种更准确的解析方法来模拟整个并合过程，减少对数值模拟的依赖。

Method: 采用混合近似方法，在不同空间区域同时应用后牛顿理论和黑洞微扰理论，并通过边界区域匹配。使用有效单体方法建模边界轨迹，演化Regge-Wheeler-Zerilli型方程，并修改Poschl-Teller势函数以匹配准正规模频率。

Result: 经过优化边界数据和势函数调整后，该方法能够匹配质量比在1到8之间的非旋转双黑洞数值相对论波形，相对误差约为10^-3量级。

Conclusion: 改进的混合方法能够准确模拟双黑洞并合波形，为引力波数据分析提供了更高效的波形模板，减少了对计算密集型数值相对论的依赖。
...
### [22] [Photon counting readout for detection and inference of gravitational waves from neutron star merger remnants](https://arxiv.org/abs/2511.09656)
*Ethan Payne,Lee McCuller,Katerina Chatziioannou*

Main category: gr-qc

TL;DR: 光子计数作为量子噪声主导数据的一种替代探测器读出方案，能够有效探测中子星并合后的引力波信号，即使信号很弱也能提供有价值的信息。


<details>
  <summary>Details</summary>
Motivation: 下一代引力波探测器预计将观测到数百万个信号，但可探测的并合后发射仍然稀少。需要探索在量子噪声主导的高频区域（1kHz以上）的替代探测方案。

Method: 采用光子计数读出方案，将信号和噪声量化为离散分布，对应在选定模式基中检测到的单个光子。通过模拟数据验证该方法的有效性。

Result: 研究发现：约1%的信噪比为0.2的并合后信号能产生单个光子而被探测到；在观测2×10^4个信号后，光子计数能使1.6M⊙中子星半径的测量精度提高两倍；降低探测器经典噪声可进一步收紧约束。

Conclusion: 光子计数为从低信噪比并合后信号中提取信息提供了一种有前景的替代方案，优于传统的零差探测技术。
...
### [23] [Finite Populations & Finite Time: The Non-Gaussianity of a Gravitational Wave Background](https://arxiv.org/abs/2511.09659)
...
TL;DR: 本文分析了脉冲星计时阵列探测到的引力波背景信号，发现现有分析中的简化假设（如高斯性假设、离散频率假设等）忽略了超大质量黑洞双星信号间的干涉效应，这些效应会引入非高斯性。


<details>
  <summary>Details</summary>
Motivation: 当前PTA数据分析对引力波背景做了过度简化假设，包括将GWB建模为高斯过程、假设SMBHBs只在观测时间的整数倍频率发射、忽略不同SMBHBs信号间的干涉效应，这些假设可能影响天体物理推断的准确性。

Method: 建立了不包含上述近似假设的天体物理GWB分析模型和数值模型，比较了其诱导的PTA信号与高斯GWB产生的信号的统计特性。

Result: 研究表明干涉效应在PTA信号中引入了非高斯性，这些效应在当前PTA分析中未被建模。

Conclusion: PTA数据分析需要考虑干涉效应引入的非高斯性，当前的分析方法存在局限性，需要更精确的模型来准确推断天体物理参数。
...
### [24] [Ladder Symmetry: The Necessary and Sufficient Condition for Vanishing Love Numbers](https://arxiv.org/abs/2511.09670)
*Chanchal Sharma,Shuvayu Roy,Sudipta Sarkar*

Main category: gr-qc

TL;DR: 本文证明在静态球对称黑洞和KRZ类旋转黑洞中，Ladder对称性是静态潮汐Love数为零的必要且充分条件。


<details>
  <summary>Details</summary>
Motivation: 研究黑洞静态潮汐Love数消失的对称性条件，探索Ladder对称性在潮汐响应中的作用。

Method: 在KRZ参数化框架下，对Ladder对称时空引入参数变形，使用参数化形式分析线性潮汐响应。

Result: 任何偏离Ladder对称背景的变形都会导致非零静态标量潮汐Love数。
...
### [25] [Periodic orbits and observational accretion disk around a Schwarzschild-like black hole surrounded by dark matter halo](https://arxiv.org/abs/2511.10043)
*Javokhir Sharipov,Tursunali Xamidov,Qiang Wu,Sanjar Shaymatov,Tao Zhu*

Main category: gr-qc

TL;DR: 研究施瓦西型黑洞在King型暗物质晕中的周期性轨道动力学和吸积盘特性，分析暗物质晕对轨道行为和辐射特征的影响。


<details>
  <summary>Details</summary>
Motivation: 探索暗物质晕环境如何影响黑洞周围的周期性轨道动力学和吸积盘的辐射特性，为理解暗物质对黑洞系统的影响提供理论依据。

Method: 通过分析不同能量和角动量配置的时间类周期性测地轨道，研究King暗物质晕对时间类周期性测地线、边际束缚轨道和最内稳定圆轨道的影响，并分析零测地线和吸积盘特性。

Result: 揭示了King暗物质晕对黑洞周围稳定轨道和粒子测地线行为的显著影响，以及吸积盘辐射通量、红移分布和观测图像的变化特征。

Conclusion: King暗物质晕显著改变了施瓦西型黑洞的时空几何结构，对吸积盘的物理和辐射特性产生重要影响，具有明确的观测意义。
...
### [26] [Veiled Singularities in Einstein-Weyl Gravity: Stability and Physical Interpretation of Horizonless Solutions](https://arxiv.org/abs/2511.10151)
*Alfio M. Bonanno,Samuele Silveravalle,Andrea Spina*

Main category: gr-qc

TL;DR: 该论文研究了爱因斯坦-外尔引力中的一类无视界解，即所谓的(-2,2)型吸引性裸奇点。与广义相对论不同，这些构型在线性张量扰动下是稳定的，所有模式都以特征振荡尾迹衰减。这些稳定的无视界构型被称为遮蔽奇点，可能代表高导数引力理论中可行的无视界黑洞替代物。


<details>
  <summary>Details</summary>
Motivation: 研究爱因斯坦-外尔引力中裸奇点的稳定性，挑战广义相对论中裸奇点不稳定且被宇宙监督假设排除的传统观点，探索高导数引力理论中无视界构型的可能性。

Method: 通过数值演化时间域中的扰动方程，分析张量扰动的线性稳定性，研究模式衰减行为和振荡尾迹特征。

Result: 发现所有扰动模式都以特征振荡尾迹衰减，表明(-2,2)型吸引性裸奇点在爱因斯坦-外尔引力中是线性稳定的，这些构型可以持续作为稳态配置存在。

Conclusion: 爱因斯坦-外尔引力中的吸引性裸奇点是动态稳定的，可以作为可行的无视界黑洞替代物，为超越爱因斯坦理论的奇点解析、稳定性和有效场动力学提供了新视角。
...
### [27] [Black-hole scattering with numerical relativity: Self-force extraction and post-Minkowskian validation](https://arxiv.org/abs/2511.10196)
*Oliver Long,Harald P. Pfeiffer,Lawrence E. Kidder,Mark A. Scheel*

Main category: gr-qc

TL;DR: 使用数值相对论模拟黑洞散射，验证自力和后闵可夫斯基展开对散射角的预测，发现二阶对称质量比（2SF）能准确再现数值相对论结果，并在弱场区域与后闵可夫斯基预测一致。


<details>
  <summary>Details</summary>
Motivation: 无束缚双黑洞相遇的渐近性质为比较广义相对论中二体问题建模的不同方法提供了清晰途径。

Method: 使用谱爱因斯坦代码生成黑洞散射的数值相对论模拟，提取自力和后闵可夫斯基展开对散射角的贡献。

Result: 二阶对称质量比（2SF）在整个质量比范围内（包括等质量情况）都能在误差范围内再现数值相对论结果；在弱场区域与后闵可夫斯基预测有良好一致性。

Conclusion: 自力和后闵可夫斯基展开方法在描述黑洞散射方面具有有效性，高阶项的相对重要性需要进一步讨论。
...
### [28] [Generating optimal Gravitational-Wave template banks with metric-preserving autoencoders](https://arxiv.org/abs/2511.10466)
*Giovanni Cabass,Digvijay Wadekar,Matias Zaldarriaga,Zihan Zhou*

Main category: gr-qc

TL;DR: 提出了一种用于构建模板库的几何放置算法，专注于引力波搜索，使用自编码器对SVD分解后的波形进行非线性压缩，通过潜在空间的均匀网格进行几何放置。


<details>
  <summary>Details</summary>
Motivation: 为引力波搜索构建高效的模板库，通过非线性压缩技术减少波形空间的维度，同时保持距离度量的一致性。

Method: 使用SVD分解将波形表示为有限基函数，然后训练自编码器进行非线性压缩，确保潜在空间和嵌入空间的距离相等，最后通过潜在空间的均匀网格进行几何放置并拒绝非物理点。

Result: 生成的模板数量略少于使用随机森林的类似构造，但在效应性测试中表现更好，特别是对于高质量双星系统。

Conclusion: 该方法在引力波搜索中有效，且维度缩减技术可应用于宇宙学领域，但周期性流形可能使寻找距离保持坐标系变得复杂。
...
### [29] [Confronting dark energy in Harada's Conformal Killing Gravity with observational data](https://arxiv.org/abs/2511.10263)
*Mohsen Khodadi,Gaetano Lambiase,Javad. T. Firouzjaee*

Main category: gr-qc

TL;DR: 基于DESI DR1、Planck CMB和Pantheon+ SN Ia数据，本研究评估了Harada共形Killing引力模型中的两种暗能量提案。纯CKG流体场景(ω=-5/3)被排除，混合模型仅勉强兼容。MCMC分析显示有效暗能量密度参数与零一致，排除了CKG所需的约5%贡献。H(z)和ω_eff(z)与ΛCDM无法区分，AIC显示无统计偏好CKG模型。


<details>
  <summary>Details</summary>
Motivation: 评估Harada共形Killing引力模型中的两种暗能量提案：纯幻影型有效暗能量(ω=-5/3)或混合场景(宇宙常数+ω=-5/3流体)，旨在解决哈勃张力和晚期加速问题。

Method: 结合DESI DR1、Planck CMB和Pantheon+ SN Ia观测数据，使用马尔可夫链蒙特卡洛(MCMC)分析约束参数，基于跨普朗克审查猜想(TCC)评估模型兼容性，通过Akaike信息准则(AIC)进行贝叶斯模型比较。

Result: 纯CKG流体场景(ω=-5/3)被排除，混合模型仅勉强兼容。有效暗能量密度参数Ω_eff=0.009^{+0.006}_{-0.007}(68% CL)，与零一致。H(z)和ω_eff(z)与ΛCDM无法区分。AIC显示无统计偏好CKG模型(ΔAIC=+2.6)。

Conclusion: Harada共形Killing引力模型中的两种暗能量提案均被当前宇宙学数据排除，哈勃张力问题仍未解决，ΛCDM模型仍是更优选择。
...
### [30] [Gravitational Atoms from Topological Stars](https://arxiv.org/abs/2511.10307)
*Ibrahima Bah,Emanuele Berti,Bogdan Ganchev,David Pereñiguez,Nicholas Speeney*

Main category: gr-qc

TL;DR: 该论文研究了拓扑星周围大质量标量场的束缚态，证明这些是严格的正规模，从而构建了真正的引力原子，清晰区分了无视界物体与黑洞。


<details>
  <summary>Details</summary>
Motivation: 研究拓扑星周围标量场束缚态的性质，以区分无视界物体与黑洞，并探索引力原子的概念。

Method: 分析大质量标量场在拓扑星周围的束缚态，考虑场的康普顿波长与星体大小的比例关系，并允许场携带电荷和Kaluza-Klein动量。

Result: 当康普顿波长远大于星体尺寸时，场形成类氢谱的云；当康普顿波长远小于星体尺寸时，场沿类时轨迹局域化；当两者尺度相当时，谱结构更丰富。

Conclusion: 拓扑星周围的标量场束缚态是严格的正规模，这为区分无视界物体与黑洞提供了明确标准，并展示了引力原子的特性。
...
### [31] [Long-lived resonances of massive scalar fields in the Reissner-Nordström black-hole spacetime: Analytic treatment in the large-mass regime](https://arxiv.org/abs/2511.10521)
*Shahar Hod*

Main category: gr-qc

TL;DR: 该研究分析了Reissner-Nordström黑洞与有质量标量场系统的物理和数学性质，发现在大质量参数Mμ≫1条件下，系统存在准共振现象，即具有任意长寿命的线性扰动模式。


<details>
  <summary>Details</summary>
Motivation: 研究带电黑洞与有质量标量场耦合系统的动力学特性，特别是探索在大质量参数条件下系统的共振行为。

Method: 使用解析技术分析Reissner-Nordström黑洞-有质量标量场系统，在无量纲大质量参数Mμ≫1条件下进行研究。

Result: 确定了黑洞-场临界质量谱{Mμ_crit(Q̅)}，该谱表征了系统长寿命共振的特性。

Conclusion: 对于给定的黑洞无量纲电荷参数Q̅，系统存在准共振现象，这些扰动模式具有任意长的寿命。
...
### [32] [Learning Post-Newtonian Corrections from Numerical Relativity](https://arxiv.org/abs/2511.10522)
*Jooheon Yoo,Michael Boyle,Nils Deppe*

Main category: gr-qc

TL;DR: 开发了一个物理信息神经网络框架，通过学习后牛顿近似与数值相对论之间的修正映射，提高引力波波形建模的准确性。


<details>
  <summary>Details</summary>
Motivation: 精确建模紧凑双星并合的引力波形对引力波天文学至关重要。后牛顿近似在早期旋进阶段有效但在合并附近失效，而数值相对论虽然准确但计算成本高且参数范围有限。

Method: 使用物理信息神经网络框架，以TaylorT4后牛顿模型为基础，在混合数值相对论代理数据上训练网络，学习轨道动力学和波形模式的高阶修正，并通过物理动机的损失项强制执行已知极限和对称性。

Result: 学习到的修正显著减少了旋进阶段到合并前约200M的相位和振幅误差。

Conclusion: 该方法提供了一个可微分且计算高效的后牛顿与数值相对论之间的桥梁，为超越现有数值相对论数据集的更稳健波形模型提供了路径。
...
### [33] [Gravitational wave polarization modes and the kinematical tensors in general relativity and beyond](https://arxiv.org/abs/2511.10567)
*Cynthia Maldonado,Francisco Nettel,Pedro A. Sánchez*

Main category: gr-qc

TL;DR: 该论文研究了运动学张量（膨胀、剪切和涡度）与引力波极化模式之间的相互关系，通过分析自由落体测试粒子在弱引力场中的响应，揭示了不同极化模式与运动学张量分量之间的对应关系。


<details>
  <summary>Details</summary>
Motivation: 探索引力波极化模式与运动学张量之间的基本联系，为理解引力波在物质中产生的几何效应提供理论基础。

Method: 通过分析自由落体测试粒子在弱引力场中的运动，研究线性化平面波在广义二阶拉格朗日量理论（包括广义相对论）中的表现。

Result: 发现剪切张量的横向分量与横向张量极化模式相关；膨胀与横向标量和纵向极化模式相关；剪切和涡度的纵向-横向分量与矢量极化模式相关。

Conclusion: 运动学张量与引力波极化模式之间存在明确的对应关系，这为通过观测测试粒子运动来识别引力波极化模式提供了理论基础。
...
### [34] [Straight and Wiggly Cosmic Strings in Horndeski Theory](https://arxiv.org/abs/2511.10578)
*M. Haluk Seçuk,Özgür Delice*

Main category: gr-qc

TL;DR: 本文研究了霍恩德斯基理论线性化版本中直宇宙弦的行为，分析了无质量和有质量标量场情况下的物理特性，并讨论了标量场质量引起的屏蔽效应。


<details>
  <summary>Details</summary>
Motivation: 研究霍恩德斯基理论中宇宙弦的行为，特别关注标量场质量对弦解的影响，以及标量场如何改变广义相对论中的标准弦解。

Method: 采用霍恩德斯基理论的线性化版本，分别考虑无质量和有质量标量场情况，分析直宇宙弦和波状宇宙弦的解析解。

Result: 发现标量场质量会产生屏蔽效应，使得有质量理论中的弦解趋近于广义相对论解；波状宇宙弦在无质量和有质量情况下也表现出类似的屏蔽效应。

Conclusion: 标量场质量在霍恩德斯基理论中起到重要的屏蔽作用，能够使弦解回归到广义相对论的预期行为，这一发现对理解修正引力理论中的宇宙弦具有重要意义。
...
### [35] [Non-stationary noise in gravitational wave analyses: The wavelet domain noise covariance matrix](https://arxiv.org/abs/2511.10632)
*Neil J. Cornish*

Main category: gr-qc

TL;DR: 本文研究了在引力波数据分析中，当噪声非平稳时，Wilson-Daubechies-Meyer小波基在何种条件下可以近似为对角噪声协方差矩阵。


<details>
  <summary>Details</summary>
Motivation: 传统傅里叶变换方法在噪声平稳时具有对角协方差矩阵的优势，但在非平稳噪声条件下该优势消失。小波域被认为更适合处理缓慢变化的非平稳噪声。

Method: 研究WDM小波包基在噪声协方差矩阵对角化方面的适用条件，分析噪声在时间和频率上缓慢变化时对角近似的有效性。

Result: 只要噪声在小波像素的时间和频率维度上都缓慢变化，WDM噪声相关矩阵就能很好地近似为对角矩阵。非对角项与动态谱模型的时间和频率导数成正比。

Conclusion: 对于在时间和频率上都具有紧支撑的小波滤波器，这种对角近似方法具有普适性。对于违反假设的快速变化噪声数据，文中讨论了相应的处理策略。
